#!/bin/bash
# Kakapos 视频运动 工具 157
# preset: video-motion-157
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_motion_157.sh"
PRESET_ID="video-motion-157"
PRESET_CATEGORY="video-motion"
PRESET_TITLE="视频运动"
...
execute_preset() {
    ...
    video_filter="setpts=PTS/1.25,fps=30"
    audio_filter="atempo=1.25"
    log_level="$(ffmpeg_log_level)"
    note "视频运动: speed=1.25x output_fps=30"
