#!/bin/bash
# Kakapos 视频运动 工具 022
# preset: video-motion-022
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_motion_022.sh"
PRESET_ID="video-motion-022"
...
execute_preset() {
    ...
    video_filter="setpts=PTS/1.5,fps=48"
    audio_filter="atempo=1.5"
    log_level="$(ffmpeg_log_level)"
    note "视频运动: speed=1.5x output_fps=48"
