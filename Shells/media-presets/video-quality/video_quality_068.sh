#!/bin/bash
# Kakapos 视频质量 工具 068
# preset: video-quality-068
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_quality_068.sh"
PRESET_ID="video-quality-068"
...
execute_preset() {
    ...
    note "视频质量: denoise=1 sharpen=0.3 crf=20"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" -vf "$filter" \
        -c:v libx264 -preset slow -crf 20 -c:a copy "${FFMPEG_WRITE_MODE[@]}" "$TEMP_OUTPUT"
}
