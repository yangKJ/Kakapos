#!/bin/bash
# Kakapos 视频质量 工具 280
# preset: video-quality-280
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_quality_280.sh"
PRESET_ID="video-quality-280"
PRESET_CATEGORY="video-quality"
PRESET_TITLE="视频质量"
...
execute_preset() {
    ...
    filter="hqdn3d=1:1:1:2,unsharp=5:5:0.3:5:5:0.0"
    log_level="$(ffmpeg_log_level)"
    note "视频质量: denoise=1 sharpen=0.3 crf=16"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" -vf "$filter" \
        -c:v libx264 -preset slow -crf 16 -c:a copy "${FFMPEG_WRITE_MODE[@]}" "$TEMP_OUTPUT"
}
