#!/bin/bash
# Kakapos 视频质量 工具 001
# preset: video-quality-001
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_quality_001.sh"
PRESET_ID="video-quality-001"
...
execute_preset() {
    ...
    filter="hqdn3d=2:2:2:4,unsharp=5:5:0.4:5:5:0.0"
    log_level="$(ffmpeg_log_level)"
    note "视频质量: denoise=2 sharpen=0.4 crf=17"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" -vf "$filter" \
        -c:v libx264 -preset slow -crf 17 -c:a copy "${FFMPEG_WRITE_MODE[@]}" "$TEMP_OUTPUT"
}
