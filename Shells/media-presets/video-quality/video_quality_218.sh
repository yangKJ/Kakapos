#!/bin/bash
# Kakapos 视频质量 工具 218
# preset: video-quality-218
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_quality_218.sh"
PRESET_ID="video-quality-218"
...
execute_preset() {
    ...
    filter="hqdn3d=3:3:3:6,unsharp=5:5:0.5:5:5:0.0"
    log_level="$(ffmpeg_log_level)"
    note "视频质量: denoise=3 sharpen=0.5 crf=18"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" -vf "$filter" \
        -c:v libx264 -preset slow -crf 18 -c:a copy "${FFMPEG_WRITE_MODE[@]}" "$TEMP_OUTPUT"
}
