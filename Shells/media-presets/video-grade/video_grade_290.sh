#!/bin/bash
# Kakapos 视频调色 工具 290
# preset: video-grade-290
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_290.sh"
PRESET_ID="video-grade-290"
...
execute_preset() {
    ...
    filter="eq=contrast=1.07:saturation=0.96:brightness=-0.01"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=1.07 saturation=0.96 brightness=-0.01"
