#!/bin/bash
# Kakapos 视频调色 工具 291
# preset: video-grade-291
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_291.sh"
PRESET_ID="video-grade-291"
...
execute_preset() {
    ...
    filter="eq=contrast=1.08:saturation=0.97:brightness=0.00"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=1.08 saturation=0.97 brightness=0.00"
