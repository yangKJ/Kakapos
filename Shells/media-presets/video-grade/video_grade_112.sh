#!/bin/bash
# Kakapos 视频调色 工具 112
# preset: video-grade-112
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_112.sh"
PRESET_ID="video-grade-112"
...
execute_preset() {
    ...
    filter="eq=contrast=0.97:saturation=1.04:brightness=-0.03"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=0.97 saturation=1.04 brightness=-0.03"
