#!/bin/bash
# Kakapos 视频调色 工具 455
# preset: video-grade-455
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_455.sh"
PRESET_ID="video-grade-455"
...
execute_preset() {
    ...
    filter="eq=contrast=1.04:saturation=1.06:brightness=-0.01"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=1.04 saturation=1.06 brightness=-0.01"
