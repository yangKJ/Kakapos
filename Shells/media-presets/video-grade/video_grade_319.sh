#!/bin/bash
# Kakapos 视频调色 工具 319
# preset: video-grade-319
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_319.sh"
PRESET_ID="video-grade-319"
...
execute_preset() {
    ...
    filter="eq=contrast=0.94:saturation=0.94:brightness=-0.05"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=0.94 saturation=0.94 brightness=-0.05"
