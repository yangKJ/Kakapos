#!/bin/bash
# Kakapos 视频调色 工具 402
# preset: video-grade-402
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_402.sh"
PRESET_ID="video-grade-402"
...
execute_preset() {
    ...
    filter="eq=contrast=0.93:saturation=1.15:brightness=0.01"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=0.93 saturation=1.15 brightness=0.01"
