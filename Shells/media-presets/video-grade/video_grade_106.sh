#!/bin/bash
# Kakapos 视频调色 工具 106
# preset: video-grade-106
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_106.sh"
PRESET_ID="video-grade-106"
PRESET_CATEGORY="video-grade"
PRESET_TITLE="视频调色"
...
execute_preset() {
    ...
    filter="eq=contrast=0.91:saturation=0.98:brightness=0.02"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=0.91 saturation=0.98 brightness=0.02"
