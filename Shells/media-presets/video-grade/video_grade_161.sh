#!/bin/bash
# Kakapos 视频调色 工具 161
# preset: video-grade-161
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_grade_161.sh"
PRESET_ID="video-grade-161"
...
execute_preset() {
    ...
    filter="eq=contrast=1.04:saturation=0.91:brightness=0.02"
    log_level="$(ffmpeg_log_level)"
    note "视频调色: contrast=1.04 saturation=0.91 brightness=0.02"
