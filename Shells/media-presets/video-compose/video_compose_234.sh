#!/bin/bash
# Kakapos 视频构图 工具 234
# preset: video-compose-234
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_compose_234.sh"
PRESET_ID="video-compose-234"
PRESET_CATEGORY="video-compose"
PRESET_TITLE="视频构图"
...
execute_preset() {
    ...
    filter="scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1"
    log_level="$(ffmpeg_log_level)"
    note "视频构图: canvas=1920x1080 letterbox=black"
