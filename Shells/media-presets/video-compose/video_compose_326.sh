#!/bin/bash
# Kakapos 视频构图 工具 326
# preset: video-compose-326
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_compose_326.sh"
PRESET_ID="video-compose-326"
...
execute_preset() {
    ...
    filter="scale=854:480:force_original_aspect_ratio=decrease,pad=854:480:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1"
    log_level="$(ffmpeg_log_level)"
    note "视频构图: canvas=854x480 letterbox=black"
