#!/bin/bash
# Kakapos 视频构图 工具 400
# preset: video-compose-400
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_compose_400.sh"
PRESET_ID="video-compose-400"
...
execute_preset() {
    ...
    filter="scale=640:360:force_original_aspect_ratio=decrease,pad=640:360:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1"
    log_level="$(ffmpeg_log_level)"
    note "视频构图: canvas=640x360 letterbox=black"
