#!/bin/bash
# Kakapos 音频交付 工具 378
# preset: audio-delivery-378
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_delivery_378.sh"
PRESET_ID="audio-delivery-378"
PRESET_CATEGORY="audio-delivery"
PRESET_TITLE="音频交付"
OUTPUT_HINT="与预设编码匹配的音频扩展名"
INPUT=""
OUTPUT=""
OVERWRITE=0
DRY_RUN=0
...
execute_preset() {
    ...
    note "音频交付: codec=flac 推荐扩展名=.flac sample_rate=44100"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" -vn \
        -ar 44100 -c:a flac -compression_level 8 "${FFMPEG_WRITE_MODE[@]}" "$TEMP_OUTPUT"
}
