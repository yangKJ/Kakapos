#!/bin/bash
# Kakapos 音频交付 工具 309
# preset: audio-delivery-309
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_delivery_309.sh"
PRESET_ID="audio-delivery-309"
...
execute_preset() {
    ...
    note "音频交付: codec=wav 推荐扩展名=.wav sample_rate=48000"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" -vn \
        -ar 48000 -c:a pcm_s16le "${FFMPEG_WRITE_MODE[@]}" "$TEMP_OUTPUT"
}
