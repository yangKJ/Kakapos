#!/bin/bash
# Kakapos 音频交付 工具 266
# preset: audio-delivery-266
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_delivery_266.sh"
PRESET_ID="audio-delivery-266"
...
execute_preset() {
    ...
    note "音频交付: codec=mp3 推荐扩展名=.mp3 sample_rate=44100"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" -vn \
        -ar 44100 -c:a libmp3lame -q:a 2 "${FFMPEG_WRITE_MODE[@]}" "$TEMP_OUTPUT"
}
