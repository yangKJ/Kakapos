#!/bin/bash
# Kakapos 相机素材接入 工具 329
# preset: camera-ingest-329
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="camera_ingest_329.sh"
PRESET_ID="camera-ingest-329"
PRESET_CATEGORY="camera-ingest"
PRESET_TITLE="相机素材接入"
OUTPUT_HINT="mp4 或 mov"
INPUT=""
OUTPUT=""
OVERWRITE=0
DRY_RUN=0
...
execute_preset() {
    ...
    filter="yadif,scale=1440:1080:force_original_aspect_ratio=decrease,pad=1440:1080:(ow-iw)/2:(oh-ih)/2:color=black,fps=30"
    log_level="$(ffmpeg_log_level)"
    note "相机素材接入: normalize=1440x1080@30fps pixel_format=yuv420p"
