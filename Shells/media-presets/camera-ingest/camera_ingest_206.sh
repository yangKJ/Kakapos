#!/bin/bash
# Kakapos 相机素材接入 工具 206
# preset: camera-ingest-206
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="camera_ingest_206.sh"
PRESET_ID="camera-ingest-206"
...
execute_preset() {
    ...
    filter="yadif,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black,fps=48"
    log_level="$(ffmpeg_log_level)"
    note "相机素材接入: normalize=1920x1080@48fps pixel_format=yuv420p"
