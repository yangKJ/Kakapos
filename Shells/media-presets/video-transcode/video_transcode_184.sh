#!/bin/bash
# Kakapos 视频转码 工具 184
# preset: video-transcode-184
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="video_transcode_184.sh"
PRESET_ID="video-transcode-184"
...
execute_preset() {
    ...
    note "视频转码: libx264 profile=main crf=22 preset=slow"
    run_command ffmpeg -nostdin -hide_banner -loglevel "$log_level" -i "$INPUT" \
        -map 0:v:0 -map 0:a? -c:v libx264 -profile:v main -crf 22 -preset slow \
