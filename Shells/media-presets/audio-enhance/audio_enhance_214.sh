#!/bin/bash
# Kakapos 音频增强 工具 214
# preset: audio-enhance-214
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_214.sh"
PRESET_ID="audio-enhance-214"
...
execute_preset() {
    ...
    filter="highpass=f=260,lowpass=f=15000,volume=1dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=260Hz lowpass=15000Hz gain=1dB"
