#!/bin/bash
# Kakapos 音频增强 工具 460
# preset: audio-enhance-460
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_460.sh"
PRESET_ID="audio-enhance-460"
...
execute_preset() {
    ...
    filter="highpass=f=140,lowpass=f=9000,volume=2dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=140Hz lowpass=9000Hz gain=2dB"
