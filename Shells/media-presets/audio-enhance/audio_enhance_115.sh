#!/bin/bash
# Kakapos 音频增强 工具 115
# preset: audio-enhance-115
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_115.sh"
PRESET_ID="audio-enhance-115"
...
execute_preset() {
    ...
    filter="highpass=f=200,lowpass=f=15000,volume=0dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=200Hz lowpass=15000Hz gain=0dB"
