#!/bin/bash
# Kakapos 音频增强 工具 024
# preset: audio-enhance-024
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_024.sh"
PRESET_ID="audio-enhance-024"
...
execute_preset() {
    ...
    filter="highpass=f=60,lowpass=f=14000,volume=0dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=60Hz lowpass=14000Hz gain=0dB"
