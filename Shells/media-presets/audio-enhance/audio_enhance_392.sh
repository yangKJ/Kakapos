#!/bin/bash
# Kakapos 音频增强 工具 392
# preset: audio-enhance-392
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_392.sh"
PRESET_ID="audio-enhance-392"
...
execute_preset() {
    ...
    filter="highpass=f=220,lowpass=f=13000,volume=-3dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=220Hz lowpass=13000Hz gain=-3dB"
