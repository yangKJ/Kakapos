#!/bin/bash
# Kakapos 音频增强 工具 465
# preset: audio-enhance-465
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_465.sh"
PRESET_ID="audio-enhance-465"
...
execute_preset() {
    ...
    filter="highpass=f=240,lowpass=f=14000,volume=0dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=240Hz lowpass=14000Hz gain=0dB"
