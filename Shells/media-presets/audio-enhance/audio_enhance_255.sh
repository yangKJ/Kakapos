#!/bin/bash
# Kakapos 音频增强 工具 255
# preset: audio-enhance-255
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_255.sh"
PRESET_ID="audio-enhance-255"
...
execute_preset() {
    ...
    filter="highpass=f=120,lowpass=f=11000,volume=0dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=120Hz lowpass=11000Hz gain=0dB"
