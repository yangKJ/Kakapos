#!/bin/bash
# Kakapos 音频增强 工具 063
# preset: audio-enhance-063
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_063.sh"
PRESET_ID="audio-enhance-063"
...
execute_preset() {
    ...
    filter="highpass=f=120,lowpass=f=8000,volume=-3dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=120Hz lowpass=8000Hz gain=-3dB"
