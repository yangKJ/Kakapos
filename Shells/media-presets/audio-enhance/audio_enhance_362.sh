#!/bin/bash
# Kakapos 音频增强 工具 362
# preset: audio-enhance-362
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="audio_enhance_362.sh"
PRESET_ID="audio-enhance-362"
...
execute_preset() {
    ...
    filter="highpass=f=100,lowpass=f=10000,volume=2dB"
    log_level="$(ffmpeg_log_level)"
    note "音频增强: highpass=100Hz lowpass=10000Hz gain=2dB"
