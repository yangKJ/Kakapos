#!/bin/bash
# Kakapos 媒体管线检查 工具 006
# preset: media-pipeline-006
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="media_pipeline_006.sh"
PRESET_ID="media-pipeline-006"
...
execute_preset() {
    ...
    video_codec="$(media_primary_codec v:0)"
    audio_codec="$(media_primary_codec a:0)"
    note "媒体管线检查: profile=6 video=${video_codec:-none} audio=${audio_codec:-none}"
    if (( DRY_RUN )); then
