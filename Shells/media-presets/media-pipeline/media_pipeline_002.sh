#!/bin/bash
# Kakapos 媒体管线检查 工具 002
# preset: media-pipeline-002
# 此文件自包含：可脱离目录独立运行，不依赖共享 shell runtime。
set -euo pipefail

PATH="${PATH:+$PATH:}/usr/bin:/bin:/usr/sbin:/sbin"
SCRIPT_NAME="media_pipeline_002.sh"
PRESET_ID="media-pipeline-002"
PRESET_CATEGORY="media-pipeline"
PRESET_TITLE="媒体管线检查"
OUTPUT_HINT="json"
INPUT=""
OUTPUT=""
...
execute_preset() {
    ...
    video_codec="$(media_primary_codec v:0)"
    audio_codec="$(media_primary_codec a:0)"
    note "媒体管线检查: profile=2 video=${video_codec:-none} audio=${audio_codec:-none}"
    if (( DRY_RUN )); then
